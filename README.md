...
Paper fills include conservative execution friction by default while staying strictly paper-only. A signal-time opportunity is rechecked through simulated latency, signing delay, queue depth, throttle pressure, adverse selection, partial fills, and deterministic submit/accept/fill/cancel failure stages before state is mutated. When a fill-time cache is available, the runner re-reads current YES/NO ask books at the simulated fill timestamp; otherwise it applies deterministic paper adjustments to the signal-time books. Completed paper fills store a top-level `simulation` object with seed, latency, queue, throttle, adverse-selection, book-source, fill timestamp, and partial-fill metadata. Simulated failures append `paper_portfolio_execution_failed` audit events and do not mutate the portfolio.
...
- `data/paper_portfolio_instance.json`: current cash, equity, realized PnL, costs, executions, inventory, book fingerprints, simulation metadata, and run metadata.
