...
`data/paper_portfolio_runtime.json` is local operational metadata written by `run` without acquiring another lock. It records host, PID, heartbeat, `warmup` / `online` / `stopping` phase, cache progress, last cycle summary, and the latest error. `poly-cond-arb status` watches that file every 2 seconds by default and repaints the same terminal dashboard panel instead of appending repeated snapshots. Each dashboard includes a UTC `Last refreshed` timestamp and renders `ONLINE`, `WARMUP`, or `DEAD`; `--refresh-seconds N` changes the watch cadence, and `--once` remains the stable single-snapshot mode for scripts or quick checks.
