...
Paper-only scanner for Polymarket same-condition arbitrage. It discovers all active Gamma events, reads public CLOB order books, evaluates standard binary YES+NO complete-set opportunities, and optionally solves neg-risk event groups with a linear program.
...
```powershell
uv run poly-cond-arb-scan --poll-interval-seconds 60
```
...
`COND_ARB_MARKET_LIMIT=0` means no local validation cap. Gamma discovery is always requested with `closed=false` and without weather or tag filtering.
